...
class SelectAndJoinBuilder
{
    ...
    public static function doclogSelectBuilder(&$query)
    {
        ...
        ->addselect(DB::raw("CASE 
            WHEN DocLog.MatterId > 0 THEN CONCAT('<span onclick=\"editRecord(''globalMatterForm'',', MatterId,')\" title=\"View the Matter\" class=\"cp mr-1 lookup-wrapper\">',CONCAT(Matter.FileRef,' - ',Matter.Description),'</span>') 
            WHEN DocLog.PartyId > 0 THEN  CONCAT('<span onclick=\"editRecord(''globalPartyForm'',', PartyId,')\" title=\"View the Party\" class=\"cp mr-1 lookup-wrapper\">',CONCAT(Party.MatterPrefix,' - ',Party.Name),'</span>') 
            ELSE '' END
            AS parentLink"))
