...
class CreateModelAndControllersControllerold extends Controller
{
    ...
    public function CreateDateFormats(Request $request)
    {
        ...
        foreach ($tableList as $TableName) {
            ...
            $dateFormatContent = "\t\tpublic static function Default". $TableName ."SelectBuilder(&\$query)\n";
            $dateFormatContent .=  "\t\t{\n\n";
                    ...
                    //     foreach ($queryRecordID as $value) {
                        ...
                        // AND is_computed = 0;
                          ...
                          //  return '$autoIncrementFlag = ' .$autoIncrementFlag .'---- $doesTableHaveIdenityColumn = '. $doesTableHaveIdenityColumn[0]->ColumnName;
                            ...
                            $classFile = fopen("ModelsAndControllers/CreatedModels/" . $TableName . ".php", "w") or die("Unable to open file!");
                            ...
                            $classFileContent .= "\n";
                            $classFileContent .= "class " . $TableName . " extends Model\n";
                            $classFileContent .= "{\n";
                            $classFileContent .= "\n";
                            ...
                            if (count($recordID) == 1) {
                                ...
                                $controllerFileContent .= "\n";
                                $controllerFileContent .= "use App\TableModels\\" . $TableName . ";\n";
                                ...
                                $controllerFileContent .= "{\n";
                                $controllerFileContent .= "\n";
                                // Removed as believed to be bad practice.
                                // $controllerFileContent .= "    public function options(Request \$request)\n";
                                // $controllerFileContent .= "    {\n";
                                // $controllerFileContent .= "        \$model = new " . $TableName . "();\n";
                                // $controllerFileContent .= "        return \$model->getFillable();\n";
                                // $controllerFileContent .= "    }\n";
                                // $controllerFileContent .= "\n";
                                ...
                                $controllerFileContent .= "\n";
                                $controllerFileContent .= "\t\t\tif (method_exists('App\Custom\QueryBuilder', 'Default" . $TableName . "JoinBuilder') & !\$request->input('removejoinbuilder')) {\n";
                                $controllerFileContent .= "\n";
                                $controllerFileContent .= "\t\t\t\tQueryBuilder::Default" . $TableName . "JoinBuilder(\$query);\n";
                                $controllerFileContent .= "\n";
                                $controllerFileContent .= "\t\t\t} \n";
                                $controllerFileContent .= "\t\t\tif (method_exists('App\Custom\QueryBuilder', 'Default" . $TableName . "SelectBuilder') & !\$request->input('select') & !\$request->input('selectraw') & !\$request->input('columnraw') & !\$request->input('column')) {\n";
                                $controllerFileContent .= "\n";
                                $controllerFileContent .= "\t\t\t\tQueryBuilder::Default" . $TableName . "SelectBuilder(\$query);\n";
                                $controllerFileContent .= "\n";
                                ...
                                if(count($recordID) > 1){
                                    ...
                                    $controllerFileContent .= "\n";
                                    $controllerFileContent .= "        return \"".$TableName;
                                    // return str_replace("'", "*", strtolower(implode(",", $recordID)));
                                ...
                                // unset(\$request[".strtolower(implode(",\n", $recordID))."]);\n";
                                // $controllerFileContent .= "        \$query->where('" . $TableName . ".RecordID', \$request->id);\n";
                                $controllerFileContent .= "\n";
                                $controllerFileContent .= "\t\t\tif (\$request->input('method')) {\n";
                                $controllerFileContent .= "\n";
                                $controllerFileContent .= "\t\t\t\t\treturn ControllerHelper::MethodHelper(\$query, \$request);\n";
                                ...
                                // $classFileContent .= "    public \$incrementing = " . ($autoIncrementFlag ? 'true' : 'false'). ";\n";
                                $controllerFileContent .= "\n";

                                // =========
                                // $controllerFileContent .= "            \$RecordID = " . $TableName . "::max('RecordID');\n";
                                // $controllerFileContent .= "\n";
                                // if($autoIncrementFlag){$controllerFileContent .= (boolval($autoIncrementFlag) ? '' : '//'). "            unset(\$request['".strtolower($doesTableHaveIdenityColumn[0]->ColumnName)."']);\n";}
                                // if($autoIncrementFlag){$controllerFileContent .= "\n";}
                                // $controllerFileContent .= "            \$createData = \$request->all();\n";
                                // // $classFileContent .= "    public \$incrementing = " . (boolval($autoIncrementFlag) ? 'true' : 'false'). ";\n";
                                // $controllerFileContent .= (boolval($autoIncrementFlag) ? '//' : ''). "            \$createData['recordid'] = \$RecordID + 1;\n";
                                // $controllerFileContent .= "\n";
                                ...
                                // $controllerFileContent .= "\n";
                                // $controllerFileContent .= "\t\t\t".($autoIncrementFlag ? '' : '//'). "\t\$recordData['".$columnName."'] = strval(\$recordData['".$columnName."']);\n";
                                $controllerFileContent .= "\n";
                                $controllerFileContent .= "\t\t\t}";
                                $controllerFileContent .= "\n";
                            ...
                            // if($autoIncrementFlag)$controllerFileContent .= (boolval($autoIncrementFlag) ? '' : '//'). "            unset(\$request[".strtolower(implode(",\n", $recordID))."]);\n";
                            // if (!empty($recordID)) {
                            //     $classFileContent .= "    protected \$primaryKey = " . implode(",\n", $recordID) . ";\n";
                            // }
                            ...
                            $controllerFileContent .= "\t\t\t\$requestData = \$request->all();\n";
                            $controllerFileContent .= "\n";
                            // if (!array_key_exists('recordid', \$requestData))
                            foreach($recordID as $value){
                            ...
                            // $controllerFileContent .= "\t\t\tif (!array_key_exists('".$columnName."', \$requestData)) \$requestData['$columnName'] = \$request->".$columnName.";\n";
                            $controllerFileContent .= "\n";
                            if(count($recordID) === 1){
                                // $controllerFileContent .= "            \$recordData->save();\n";
                                $controllerFileContent .= "\n";
                            ...
                            $controllerFileContent .= "        return ControllerHelper::tryCatch(\$request, function (\$request) {\n";
                                ...
                                // $controllerFileContent .= "            ".$TableName . "::findOrFail(\$request->id)->delete();\n";
                                // $controllerFileContent .= "\t\t\t\t\$recordData['data'] = [];\n";
                                $controllerFileContent .= "\n";
                                $controllerFileContent .= "\t\t\t}\n";
                                $controllerFileContent .= "\n";
                                $controllerFileContent .= "        return ControllerHelper::PostPutDeleteFormatHelper(\$recordData, \$request);\n";
