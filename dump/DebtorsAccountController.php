...
use App\TableModels\IntRatePeriod;
use App\TableModels\Language;
use App\TableModels\Matter;
use App\TableModels\ColDebit;
use App\TableModels\ColData;
use App\TableModels\CommissionRate;
...
class DebtorsAccountController extends Controller
{
    ...
    public function getDebtorsAccount($request) {
        ...
        try {
            ...
            if ( $this->debugging ) $startTime = microtime(true);

            /*Testing
			$request = new stdClass();$request->matterid = 93629;$controller = new \App\Http\Controllers\DebtorsAccountController;$controller->debugging = true;$controller->getDebtorsAccount($request);
			$request = new stdClass();$request->matterid = 93540;$controller = new \App\Http\Controllers\DebtorsAccountController;$controller->debugging = true;$controller->getDebtorsAccount($request);
			$request = new stdClass();$request->matterid = 3077;$controller = new \App\Http\Controllers\DebtorsAccountController;$controller->debugging = true;$controller->getDebtorsAccount($request);
			$request = new stdClass();$request->matterid = 3047;$controller = new \App\Http\Controllers\DebtorsAccountController;$controller->debugging = true;$controller->getDebtorsAccount($request);
            */
            ...
            if (!isset($matterId)) {
                throw new \Exception('No MatterID specified');
            }
            ...
            if ( !$this->colData ) throw new \Exception("An error was encountered getting the ColData table for this Matter.");
            ...
            $this->TotalCapitalBalance = $this->matter->InterestOnAmount ? $this->matter->InterestOnAmount : $this->matter->DebtorsOpeningBalance;
            ...
            // if ( $this->debugging ) {
                ...
                //echo "\nthis->matter->ReceiptPercentToCosts = " . floatval($this->matter->ReceiptPercentToCosts);
                //echo "\nthis->colData->ReceiptPercentToCostsFlag = " . intval($this->colData->ReceiptPercentToCostsFlag);
                //echo "\nthis->matter->ReceiptPercentToDate = " . intval($this->matter->ReceiptPercentToDate);
                //echo "\nthis->colData->MaxCommission = " . floatval($this->colData->MaxCommission);
                //echo "\nthis->matter->DebtorCollCommLimit = " . floatval($this->matter->DebtorCollCommLimit);
                 //return;
            ...
            if ( $this->debugging ) {
                ...
                for ($i=0; $i < count($colDebits); $i++) { 
                    echo "\n" .Utils::stringFromClarionDate($colDebits[$i]->date) . " " . $colDebits[$i]->type . " " . $colDebits[$i]->timerstamp . " " . $colDebits[$i]->description  . " " . $colDebits[$i]->amount; 
                }
            ...
            //     echo "\n\nAfter adding Commission transactions\n";

            //     for ($i=0; $i < count($this->colDebitQueue); $i++) { 
            //         echo "\n" .Utils::stringFromClarionDate($this->colDebitQueue[$i]->date) . " " . $this->colDebitQueue[$i]->type . " " . $this->colDebitQueue[$i]->timerstamp . " " . $this->colDebitQueue[$i]->description  . " " . $this->colDebitQueue[$i]->totalamount; 
            //     }

            // }


            $this->insertInterestTransactions( $request );

            $this->sortColDebit();

            if ( $this->debugging ) {

                echo "\n\nAfter adding Interest transactions\n";

                for ($i=0; $i < count($this->colDebitQueue); $i++) { 
                    echo "\n" .Utils::stringFromClarionDate($this->colDebitQueue[$i]->date) . " " . $this->colDebitQueue[$i]->type . " " . $this->colDebitQueue[$i]->timerstamp . " " . $this->colDebitQueue[$i]->recordid; 
                    if ( isset($this->colDebitQueue[$i]->description) ) {
                        echo " " . $this->colDebitQueue[$i]->description;
                    } else if ( $this->colDebitQueue[$i]->type ==='I')  {
                        echo " Interest Placeholder";
                    } else  {
            ...
            foreach ($this->colDebitQueue as $colDebit) {
                $this->formatColDebit($colDebit);
            }
            ...
            if ( $this->debugging ) {
                ...
                for ($i=0; $i < count($this->colDebitQueue); $i++) { 
                    echo "\n" .Utils::stringFromClarionDate($this->colDebitQueue[$i]->date) . " " . $this->colDebitQueue[$i]->type . " " . $this->colDebitQueue[$i]->timerstamp . "\n" . $this->colDebitQueue[$i]->description  . " " . $this->colDebitQueue[$i]->totalamount; 
                    echo "\n\t\t\t\t\t\t\t\t\tCostBalance:" . $this->colDebitQueue[$i]->costbalance . "\tInterestBalance:" . $this->colDebitQueue[$i]->interestbalance . "\tCapital Balance:" . $this->colDebitQueue[$i]->capitalbalance;//  . " " . $this->colDebitQueue[$i]->balance; 
                }

                echo "\n\nProcessing " . count($this->colDebitQueue);
                echo " records took " . number_format($totalExecutionTime,2) . " seconds\n";
    ...
    private function insertInterestTransactions( $request ) {
        ...
        if ( isset($request->interestDate) ) {
            $this->InterestToDate = $request->interestDate; 
        }
        ...
        while (!$BreakLoop) {
            ...
            $this->addInterestToColDebitQueue();
    ...
    private function addInterestToColDebitQueue() {
        ...
        if ( !$this->matter->InterestCompoundedFlag ) {
            ...
            if ($this->TheNextInterestDate > $this->InterestToDate)  {
                $this->TheNextInterestDate = $this->InterestToDate;
            }
        ...
        $this->addColDebit($interestRecord);
    ...
    private function calculateBalances() {
        ...
        if ( !$this->RunningCapitalBalance ) $this->RunningCapitalBalance = $this->matter->DebtorsOpeningBalance;

        $this->addOpeningBalance();

        foreach ($this->colDebitQueue as $colDebit) {

            // echo "\n\n######### LOOPING THRU colDebitQueue AND CALLING adjustTheBalances";
            // echo "\ncolDebit->type = $colDebit->type";
            // echo "\ncolDebit->date = $colDebit->date";
            // echo "\ncolDebit->timerstamp = $colDebit->timerstamp";
            // echo "\n##################\n";
        ...
        if ($colDebit->type == 'D') {   // COST
            ...
            if ( $this->CalculateInterestOn > 0 && $colDebit->interestrate) {
                ...
                if ( $noOfDays > 0 ) {
                    ...
                    //echo "\ncolDebit->interestrate = " . $colDebit->interestrate;
                ...
                if ( $colDebit->amount && floatval($this->colData->InterestEndAmount) ) {
    ...
    private function addOpeningBalance() {
        ...
        array_unshift($this->colDebitQueue, $record);
    ...
    public function getCurrentInterestRate($date) {
        ...
        if ( intval($this->matter->IntRateScheduleID) ) {
            ...
            $IntRatePeriod = IntRatePeriod::select('InterestRate')
            ->whereRaw("IntRateScheduleID = " . $this->matter->IntRateScheduleID . " AND FromDate <= $date")
            ->orderBy('FromDate','desc')->first();
    ...
    private function getNextScheduleDate($date) {
        ...
        //RowCounter{PROP:SQL} = 'SELECT TOP 1 ToDate FROM IntRatePeriod WHERE ToDate >= ' & TheDate & ' AND IntRateScheduleID = ' & MAT:IntRateScheduleID & ' ORDER BY ToDate'

        //echo "\ngetNextScheduleDate whereRaw = ToDate >= $date AND IntRateScheduleID = $this->matter->IntRateScheduleID";
        $IntRatePeriod = IntRatePeriod::select('ToDate')->whereRaw("ToDate >= $date AND IntRateScheduleID = " . $this->matter->IntRateScheduleID)->orderBy('ToDate')->first();
    ...
    private function getCollCommAmount($date, $amount, $vatRate) {
        ...
        try {
            ...
            throw new \Exception('Server Error on line '.$e->getLine().' in '.$e->getFile() .': <b>'.$e->getMessage());
        }
    ...
    private function getCollCommPercent($date) {
        ...
        try {
            ...
            $record = CommissionRate::select('commission')
            ->whereRaw($date . ' >= FromDate AND ' . $date .' <= ToDate')
            ->first();
    ...
    private function getCollCommLimit($date) {
        ...
        try {
            ...
            $record = CommissionRate::select('limit')
            ->whereRaw($date . ' >= FromDate AND ' . $date .' <= ToDate')
            ->first();
    ...
    private function getVatAmount($Amount,$VatPercent,$VatRate,$VatIE) {
    ...
    private function formatColDebit(&$colDebit) {

        $colDebit->formatteddate = $colDebit->date ? Utils::stringFromClarionDate($colDebit->date) : '';
    ...
    private function addColDebit($colDebit) {
        ...
        $this->colDebitQueue[] = $colDebit;

    }

    public function printColDebitQueue() {
        print_r($this->colDebitQueue);
    }

    private function sortColDebit() {

        // Sort by Date and TimerStamp
        if (!empty($this->colDebitQueue)) {

            usort($this->colDebitQueue, function($a, $b) { 
        ...
        /* FOR TESTING
            ...
            $colDebitQueue = [];
            $record = new \stdClass();$record->description = 'Interest';$record->timerstamp = 0; $record->date = 80631;array_push($colDebitQueue,$record);
            $record = new \stdClass();$record->description = 'Payment';$record->timerstamp = 10; $record->date = 80636;array_push($colDebitQueue,$record);
            $record = new \stdClass();$record->description = 'Commission';$record->timerstamp = 11; $record->date = 80636;array_push($colDebitQueue,$record);
            $controller->sortColDebit();
            print_r($colDebitQueue);
