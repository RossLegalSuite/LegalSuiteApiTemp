...
class DebtorsAccountController extends Controller
{
    ...
    public function getDebtorsAccount($request)
    {
        try {
            ...
            */
            ...
            if (! $this->colData) {
                throw new \Exception('This Matter does not have a Debtors Account.');
            }
            ...
            if (! $this->control) {
                throw new \Exception('An error was encountered getting the Control table.');
            }
    ...
    private function insertInterestTransactions()
    {
        try {
            ...
            if (! intval($this->InterestFromDate)) {
                return;
            }
            if (! intval($this->InterestToDate)) {
                return;
            }
            if ($this->InterestFromDate >= $this->InterestToDate) {
                return;
            }
    ...
    private function addInterestToQueue()
    {
        try {
            if (! $this->matter->InterestCompoundedFlag) {
                ...
                if (($this->TheNextInterestDate - $this->LastInterestDate) > 62) {
                    ...
                    if (! intval($this->SavedDate)) {
                        $this->SavedDate = $this->LastInterestDate;
                    }
            ...
            $interestRecord->amount = 0;
            $interestRecord->vatamount = 0;
            $interestRecord->totalamount = 0;
    ...
    private function calculateBalances()
    {
        try {
            ...
            if (! floatval($this->RunningCapitalBalance)) {
                $this->RunningCapitalBalance = $this->matter->DebtorsOpeningBalance;
            }
    ...
    private function adjustTheBalances(&$colDebit)
    {
        try {
            ...
            if ($colDebit->type == 'D') {   // COST
            ...
            } elseif ($colDebit->type == 'W') {   // COLLECTION COMMISSION ADDED BY THE USER
                ...
                $this->updateTransactionBalances($colDebit);
            } elseif ($colDebit->type == 'X') {   // COLLECTION COMMISSION ADDED BY THE PROGRAM
            ...
            } elseif ($colDebit->type == 'J') {   // INTEREST MANUALLY INSERTED BY USER
                ...
                $this->updateTransactionBalances($colDebit);
            } elseif ($colDebit->type == 'V') {  // EMPLOYER'S COMMISSION
                ...
                $this->addVirtualTransaction($colDebit);
            } elseif ($colDebit->type == 'P') {    // PAYMENT
    ...
    private function adjustCostsForInduplumRule(&$colDebit)
    {
        ...
        if (intval($this->colData->NewInDuplumRuleFromDate) && $colDebit->date < $this->colData->NewInDuplumRuleFromDate) {
            return;
        }
    ...
    public function getCurrentInterestRate($date)
    {
        ...
        if (intval($this->matter->IntRateScheduleID)) {
            ...
            $intRatePeriod = $this->intRatePeriods->last(function ($value, $key) use ($date) {
                return $value->FromDate <= intval($date);
            });
    ...
    private function reduceCapitalBalance()
    {
        ...
        } elseif ($this->colData->ReduceCapitalFlag == 2) {
        ...
        } elseif ($this->colData->ReduceCapitalFlag == 3) {
            $this->RunningCapitalBalance -= $this->AmountToReduceCapitalBy;
        ...
        } elseif ($this->colData->ReduceCapitalFlag == 4) {
            $this->InterestBalance -= $this->AmountToReduceCapitalBy;
    ...
    private function getCollCommAmount($date, $amount, $vatRate)
    {
        try {
            ...
            if (! floatval($amount) || $this->matter->DebtorCollCommOption == 'N') {
                return $returnValue;
            }
            ...
            } elseif ($this->matter->DebtorCollCommOption == 'S') {
    ...
    private function getCollCommMatterPercent($date)
    {
        try {
            ...
            if ($this->matter->DebtorCollCommOption == 'N') {
                $returnValue = 0;
            } elseif ($this->matter->DebtorCollCommOption == 'U') {
    ...
    private function setCurrentVatRate($date)
    {
        try {
            ...
            if ($this->matter->VatExemptFlag) {
                $returnValue = 0;
            } elseif ($this->control->VatMethod != 'N') {
                if ($date < 79352) {
    ...
    private function getEmployerCommAmount($colDebit)
    {
        try {
            ...
            if (! intval($this->colData->EMOFirstDate)) {
                return $returnValue;
            }

            if ($colDebit->date < $this->colData->EMOFirstDate) {
                return $returnValue;
            }

            if ($this->colData->EMOEndDate > 0 && $colDebit->date > $this->colData->EMOEndDate) {
                return $returnValue;
            }

            if (! floatval($this->colData->EMOCommissionPercent)) {
                return $returnValue;
            }
