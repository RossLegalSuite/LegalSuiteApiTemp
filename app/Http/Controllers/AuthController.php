...
class AuthController extends Controller
{
    public function auth(Request $request)
    {
        try {
            if (! $request->header('Authorization')) {
                return json_encode(['error' => 'No Developer Token specified']);
            }
            ...
            if (! $request->company) {
                return json_encode(['error' => 'No Company Code specified']);
            }
            if (! $request->login) {
                return json_encode(['error' => 'No Login specified']);
            }

            $company = Client::where('firmcode', $request->company)->first();

            if (! $company) {
                return json_encode(['error' => 'Company ('.$request->company].') not found');
            }
            ...
            if (! $apiAccessDetails) {
                ...
                if ($apiAccessDetails->grantedAccess = 0) {
                    return json_encode(['error' => 'API Access to the Company has not been granted']);
                }
            }
            ...
            if (is_null($thisEmployee)) {
                return json_encode(['error' => 'Invalid Username or Incorrect Password']);
            }

            if ($thisEmployee->suspendedflag !== '0') {
                return json_encode(['error' => 'This User is Suspended']);
            }
            ...
            if (isset($request->docgencode) && ($request->docgencode === 'LOLNEW' || $request->docgencode === 'MOBILE')) {
                ...
                if (! $docgenRecord) {
                    ...
                    } elseif ($request->docgencode === 'MOBILE') {
                    ...
                    try {
                        ...
                        */
                        ...
                        if ($licenceResponse->ApplyForLegalSuiteLicencesResult == '-1') {
                            return json_encode(['error' => 'An Error was encountered adding the Licence details to the LegalSuite Client Database']);
                        }
                    } catch (\Exception $e) {
                        return json_encode(['error' => 'An Error was encountered accessing the WSDL Licensing system: '.$e->getMessage()]);
                    }
                } else {
                    ...
                    if (! $licensedRecord) {
                        if ($request->docgencode === 'LOLNEW') {
                            return json_encode(['error' => '<p>This User is not licensed to use LegalSuite Online.</p><p>Please add this Employee to the list of licensed Users for the Document Set (LOLNEW).</p>']);
                        } elseif ($request->docgencode === 'MOBILE') {
                            return json_encode(['error' => 'This User is not licensed to use the LegalSuite Mobile App. Please add this Employee to the list of licensed Users for the Document Set (MOBILE).']);
                        }
                    }
                }
            }
            ...
            return json_encode(['error' => 'Server Error on line '.$e->getLine().' in '.$e->getFile().': <br/>'.$e->getMessage()]);
        }
    }
    ...
    public function getApiKey(Request $request)
    {
        try {
            if (! $request->header('Authorization')) {
                return json_encode(['error' => 'Developer Key in Authorization header not specified']);
            }

            if (! Developer::where('developer_token', $request->header('Authorization'))->first()) {
                return json_encode(['error' => 'Developer not found']);
            }

            if (! $request->company) {
                return json_encode(['error' => 'Company Code not specified']);
            }
