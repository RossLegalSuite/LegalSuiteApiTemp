...
class MatterRulesController
{
    ...
    private function initializeData(&$matter, $employeeId) {
        ...
        try {
            ...
            if ( !$control ) throw new \Exception("An error was encountered getting the Control table. It is required to initialize the Matter record.");

            $employee = employee::select(['branchid','feeestimatewarningflag'])->where('RecordID',$employeeId)->first();
            if ( !$employee ) throw new \Exception("An error was encountered getting the Branch ID from the Employee table. It is required to initialize the Matter record.");
            ...
            if ( $lastMatterAdded ) {
                ...
                if ( !isset($matter->collcommoption) ) $matter->collcommoption = $lastMatterAdded->collcommoption;
                if ( !isset($matter->debtorcollcommoption) ) $matter->debtorcollcommoption = $lastMatterAdded->debtorcollcommoption;
    ...
    private function sanitizeData(&$matter, $employeeId) {
        ...
        try {
            ...
            if ( !$matter->branchflag ) $matter->branchid = null;
            ...
            if ( isset($matter->collcommoption) || isset($matter->debtorcollcommoption) ) {
                ...
                if ( !$control ) throw new \Exception("An error was encountered getting the Control table. It is required to check the Matter data.");

                if ( isset($matter->collcommoption) ) {
                    if ( $matter->collcommoption == 'U' ) {
                        $matter->collcommpercent = $control->collcommpercent;
                        $matter->collcommlimit   = $control->collcommlimit;
                    } else if ( $matter->collcommoption == 'N' ) {
                        $matter->collcommpercent = 0;
                        $matter->collcommlimit = 0;
                    }
                }
    ...
    private function addMatterParties($matter) {
        ...
        try {
            ...
            if ( !$control ) throw new \Exception("An error was encountered getting the Control table. It is required to add the default Parties to the Matter.");
            ...
            } else if ( $this->isTransfersMatter($matter) ) {
            ...
            } else  if ( $this->isBondMatter($matter) ) {
            ...
            } else  if ( $this->isSectionalTitleRegisterMatter($matter) ) {
    ...
    private function checkArchiveNumber($matter) {
        ...
        try {
            ...
            if ( $matter->archivestatus == '2' ) {
                ...
                } else if ( ($matter->consolidatedflag == '1' || $matter->consolidatedflag == '2') && 
                    ($matter->actual != 0 || 
                    $matter->reserved != 0 || 
                    $matter->totalfees != 0 || 
                ...
                } else if ( $matter->createinvoiceflag == '1' && $matter->invoiceFlag == '1') {
    ...
    private function getNextArchiveNo() {
        ...
        try {
            ...
            if ($maxArchiveNumber < 0) $maxArchiveNumber = 0;
            ...
            if ( $control ) {
                ...
                } else if ( $lastArchiveId >= $returnValue ) {
                    $lastArchiveId += 1;
                    $returnValue = $lastArchiveId;
                }

                control::where('recordid', $control->recordid)
                ->update(array('lastArchiveId' => $lastArchiveId));
    ...
    private function isCollectionsMatter($matter) {
        ...
        if ( !isset($matter->docgentype) ) $this->getDocgen($matter);
