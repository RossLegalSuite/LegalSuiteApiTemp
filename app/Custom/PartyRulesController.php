...
class PartyRulesController
{
    ...
    private function sanitizeData(&$party, &$parlang) {
        ...
        // Convert dates
        // ******************************************
        if ( isset($parlang->trustdate) ) $parlang->trustdate = ModelHelper::convertClarionDate($parlang->trustdate);

        if ( isset($parlang->marriagedate) ) $parlang->marriagedate = ModelHelper::convertClarionDate($parlang->marriagedate);

        if ( isset($parlang->birthdate) ) $parlang->birthdate = ModelHelper::convertClarionDate($parlang->birthdate);

        if ( isset($parlang->spousebirthdate) ) $parlang->spousebirthdate = ModelHelper::convertClarionDate($parlang->spousebirthdate);

        if ( isset($parlang->gpasignedon) ) $parlang->gpasignedon = ModelHelper::convertClarionDate($parlang->gpasignedon);

        if ( isset($party->dateresolutionsigned) ) $party->dateresolutionsigned = ModelHelper::convertClarionDate($party->dateresolutionsigned);

        if ( isset($party->remoteaccessexpiry) ) $party->remoteaccessexpiry = ModelHelper::convertClarionDate($party->remoteaccessexpiry);

        // ******************************************
        //Check data and set defaults if missing
        // ******************************************
        if ( !isset($party->defaultroleid) ) $party->defaultroleid = control::pluck('clientroleid')->first();

        if ( !isset($party->defaultlanguageid) ) $party->defaultlanguageid = control::pluck('defaultlanguageid')->first();
        ...
        if ( $this->isJursitic($party) ) {
            ...
            if ( isset($parlang->initials) ) $party->name = $parlang->name . ' ' . $parlang->initials;

            if ( isset($parlang->alternativename) ) {

                $party->name = $party->name . ' (' . $parlang->alternativename . ')';

            } else if ( isset($parlang->firstname) ) {
