...
class ToDoNoteRulesController {
    ...
    private function processCompletedReminder($request, $record) {
        ...
        if ( !$employee ) throw new \Exception("An error was encountered getting the logged in Employee record.");
    ...
    private function updateConveyancingFlags($record, $criticalStep) {
        ...
        } else if ( $criticalStep == '9' ) { // ELECTRICAL COMPLIANCE CERTIFICATE RECEIVED
        ...
        } else if ( $criticalStep == '35') {   // Plumbing COMPLIANCE CERTIFICATE RECEIVED
