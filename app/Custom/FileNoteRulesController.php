...
class FileNoteRulesController {
    ...
    private function addFeeNote($record) {
        ...
        if ( isset($record->createfeenoteflag) && isset($record->createfeenotedescription) && isset($record->createfeenoteamount) ) {
            ...
            if ($record->createfeenoteflag == '1') {
                ...
                if ( !$matter ) throw new \Exception("An error was encountered getting the VatExemptFlag from the Matter.");
                ...
                if ( !$control ) throw new \Exception("An error was encountered getting the VatMethod from the Control table.");

        
                if ( $matter->VatExemptFlag == '1') {
                    $feeNote->vatrate = 'E';
                } else if ( $matter->VatExemptFlag == '2') {
                    $feeNote->vatrate = 'Z';
                } else if ( $control->VatMethod == 'N') {
                    $feeNote->vatrate = 'N';
                } else {
                    $feeNote->vatrate = '1';
                }
    ...
    private function validateData($record) {
        ...
        if ( isset($record->stageid) && $record->stageid != '0' ) {
            ...
            if ( !$stage ) throw new \Exception("An error was encountered getting the Stage table. It is required to check the Stage conditions.");
